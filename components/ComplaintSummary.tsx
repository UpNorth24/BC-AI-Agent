...
const DetailItem: React.FC<{ label: string; value: string | boolean | null }> = ({ label, value }) => {
  ...
  return (
    <div>
      ...
      <p className={`mt-1 p-3 rounded-lg text-gray-700 ${hasValue ? 'bg-gray-200' : 'bg-gray-50 italic'}`}>
...
const LongDetailItem: React.FC<{ label: string; value: string | null }> = ({ label, value }) => {
  ...
  return (
    <div>
      <h3 className="text-sm font-semibold text-gray-500">{label}</h3>
      <p className={`mt-1 p-3 rounded-lg text-gray-700 min-h-[60px] whitespace-pre-wrap break-words ${hasValue ? 'bg-gray-200' : 'bg-gray-50 italic'}`}>
...
const FileListItem: React.FC<{ label: string; files: string[] }> = ({ label, files }) => {
    ...
    return (
      <div>
        <h3 className="text-sm font-semibold text-gray-500">{label}</h3>
        <div className={`mt-1 p-3 rounded-lg text-gray-700 ${hasFiles ? 'bg-gray-200' : 'bg-gray-50 italic'}`}>
          {hasFiles ? (
